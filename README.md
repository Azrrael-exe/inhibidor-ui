...
- **Tab Demo** — demo funcional de las capacidades del equipo, con dos botones: **stress test HTTP** (corre `tools/http_stress.py` contra el dispositivo y muestra el veredicto PASS/FAIL con el análisis completo) y **demo completa** (secuencia end-to-end: versión de firmware → estado → encendido secuencial de las 7 bandas RF [opcional] → rutina completa de movimientos con checklist en vivo — azimuth ida y vuelta, elevación ida y vuelta, movimiento combinado y homming, cada paso marcándose ✅ uno a uno con los ángulos leídos en tiempo real → hard-stop final). La emisión de RF es opt-in con un checkbox propio (deshabilitada por default, la demo solo mueve el rotor); ejecutar requiere además un checkbox de confirmación, y siempre termina en estado seguro (todo OFF). Cada request HTTP de la demo queda visible en un log expandible (hora, método, endpoint, código de respuesta y latencia). Pausá el auto-refresh mientras la usás (el refresco vuelve al tab Control).
